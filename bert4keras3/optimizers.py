...
from bert4keras.backend import keras, K, is_tf_keras,ops
...
@export_to_custom_objects
def extend_with_weight_decay_v3(BaseOptimizer):
    """返回新的优化器类，加入权重衰减
    """
    class NewOptimizer(BaseOptimizer):
        """带有权重衰减的优化器
        ...
        def update_step(self, gradient, variable, learning_rate):
            old_assign_add = self.assign_add
            old_assign_sub = self.assign_sub
            old_assign = self.assign
            
            def new_assign(x, new_x):
                # old_assign is x=new_x
                if x is variable and self._do_weight_decay(x):
                    lr_t =  ops.cast(self.weight_decay, x.dtype)
                    new_x = new_x - lr_t * self.weight_decay_rate * x
                old_assign(x, new_x)
            
            def new_assign_add(variable, value):
                # old_assign_add is variable+=value
                value = variable + value
                new_assign(variable, value)
            
            def new_assign_sub(variable, value):
                # old_assign_add is x+=new_x
                value = variable - value
                new_assign(variable, value)
            
            self.assign_add = new_assign_add
            self.assign_sub =new_assign_sub
            self.assign = new_assign
            super(NewOptimizer, self).update_step(self, gradient, variable, learning_rate)
            self.assign_add = old_assign_add
            self.assign_sub =old_assign_sub
...
@export_to_custom_objects
def extend_with_layer_adaptation_v3(BaseOptimizer):
    """返回新的优化器类，加入层自适应学习率
    """
    class NewOptimizer(BaseOptimizer):
        """带有层自适应学习率的优化器
        ...
        def update_step(self, gradient, variable, learning_rate):
            ...
            def new_assign(x, new_x):
                # old_assign is x=new_x
                if x is variable and self._do_layer_adaptation(x):
                    dx = new_x - x
                    lr_t =  ops.cast(self.weight_decay, x.dtype)
                    lr_t = ops.maximum(lr_t, K.epsilon())
                    x_norm = ops.norm(x)
                    g_norm = ops.norm(dx / lr_t)
                    ratio = ops.where(
                        x_norm > 0.0,
                        ops.where(g_norm > 0.0, x_norm / g_norm, 1.0), 1.0
                    )
                    new_x = x + dx * ratio
                old_assign(x, new_x)
...
@export_to_custom_objects
def extend_with_piecewise_linear_lr_v3(BaseOptimizer):
    """返回新的优化器类，加入分段线性学习率
    """
    class NewOptimizer(BaseOptimizer):
        """带有分段线性学习率的优化器
        ...
        def update_step(self, gradient, variable, learning_rate):
            lr_multiplier = piecewise_linear(self.iterations, self.lr_schedule)
            super(NewOptimizer, self).update_step(self, gradient, variable, lr_multiplier)
...
def extend_with_gradient_accumulation_v2(BaseOptimizer):
    ...
    return NewOptimizer
from bert4keras3 import int_shape
@export_to_custom_objects
def extend_with_gradient_accumulation_v3(BaseOptimizer):
    """返回新的优化器类，加入梯度累积
    """
    class NewOptimizer(BaseOptimizer):
        """带有梯度累积的优化器
        ...
        def update_step(self, gradient, variable, learning_rate):
            ...
            cond = ops.equal(self.iterations % self.grad_accum_steps, 0)
            cond = K.cast(cond, K.floatx())
            self.assign(self.accum_grads[variable],
                        gradient/self.grad_accum_steps+(1 - cond) * self.accum_grads[variable]
                )
            ...
            super(NewOptimizer, self).update_step(self, self.accum_grads[variable], variable, learning_rate)
            self.assign_add = old_assign_add
            self.assign_sub =old_assign_sub
            self.assign = old_assign
...
@export_to_custom_objects
def extend_with_lazy_optimization_v3(BaseOptimizer):
    """返回新的优化器类，加入懒惰更新
    """
    class NewOptimizer(BaseOptimizer):
        """带有懒惰更新的优化器
        ...
        def update_step(self, gradient, variable, learning_rate):
            ...
            def new_assign(x, new_x):
                # old_assign is x=new_x
                if x is variable and self._do_lazy_optimization(x):
                    r = ops.any(ops.not_equal(gradient, 0.0), axis=-1, keepdims=True)
                    new_x = x + (new_x - x) * ops.cast(r, K.floatx())
                old_assign(x, new_x)
...
@export_to_custom_objects
def extend_with_exponential_moving_average_v3(BaseOptimizer):
    """返回新的优化器类，加入EMA（权重滑动平均）
    keras3原生支持，不需要做额外实现
    """
    class NewOptimizer(BaseOptimizer):
...
@export_to_custom_objects
def extend_with_parameter_wise_lr_v3(BaseOptimizer):
    """返回新的优化器类，加入分参数学习率
    主要场景就是给每层甚至每个参数设置不同的学习率。
    """
    class NewOptimizer(BaseOptimizer):
        ...
        def update_step(self, gradient, variable, learning_rate):
            lr_multiplier = 1
            for k, v in self.paramwise_lr_schedule.items():
                if k in variable.name:
                    lr_multiplier *= v
                    break
            
            super(NewOptimizer, self).update_step(self, gradient, variable, learning_rate*lr_multiplier)
...
def extend_with_isnan_skip(BaseOptimizer):
    """返回新的优化器类，加入梯度累积
    """
    class NewOptimizer(BaseOptimizer):
        """带有梯度累积的优化器
        ...
        def get_updates(self, loss, params):
            ...
            def new_update(x, new_x):
                cond = tf.reduce_any(ops.is_nan(new_x))
                cond = K.cast(cond, K.floatx())
                new_x = (1 - cond) * new_x +  cond * x
                return old_update(x, new_x)
...
def extend_with_isnan_skip_v2(BaseOptimizer):
    ...
    class NewOptimizer(BaseOptimizer):
        ...
        def _resource_apply_dense(self, grad, var):
            old_update = K.update
            cond = tf.reduce_any(ops.is_nan(grad))
            cond = K.cast(cond, K.floatx())
            def new_update(x, new_x):
                new_x = (1 - cond) * new_x +  cond * x
                return old_update(x, new_x)
            K.update = new_update
            updates = super(NewOptimizer, self)._resource_apply_dense(grad, var)
            K.update = old_update
            return updates
...
@export_to_custom_objects
def extend_with_isnan_skip_v3(BaseOptimizer):
    """跳过带nan的部分
    """
    class NewOptimizer(BaseOptimizer):
        """带有梯度累积的优化器
        ...
        def update_step(self, gradient, variable, learning_rate):
            ...
            cond = ops.isnan(gradient)
            cond = ops.cast(ops.any(cond),gradient.dtype)
...
if is_tf_keras:
    ...
    extend_with_isnan_skip = extend_with_isnan_skip_v2
    Lion = LionV2
    AdaFactor = AdaFactorV2
elif keras.__version__>'3.0':
    Adam = keras.optimizers.Adam
    AdaFactor = keras.optimizers.AdaFactor
    Lion = keras.optimizers.Lion
    extend_with_weight_decay = extend_with_weight_decay_v3
    extend_with_layer_adaptation = extend_with_layer_adaptation_v3
    extend_with_piecewise_linear_lr = extend_with_piecewise_linear_lr_v3
    extend_with_gradient_accumulation = extend_with_gradient_accumulation_v3
    extend_with_lazy_optimization = extend_with_lazy_optimization_v3
    extend_with_exponential_moving_average = extend_with_exponential_moving_average_v3
    extend_with_parameter_wise_lr = extend_with_parameter_wise_lr_v3
    extend_with_isnan_skip = extend_with_isnan_skip_v3
else:
    Adam = keras.optimizers.Adam
